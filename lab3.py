...
def plot_original_ch1_ch2_mth(orig_path_ch1, orig_path_ch2, orig_path_mth, ch1_2_scale, ch1_name, ch2_name, mth_name, total_path_name):
    # Importing data from source CSVs
    ch1 = pd.read_csv(orig_path_ch1, usecols=[3,4], index_col=[0], header=None, names=['t (s)', ch1_name])
    ch2 = pd.read_csv(orig_path_ch2, usecols=[3,4], index_col=[0], header=None, names=['t (s)', ch2_name])
    mth = pd.read_csv(orig_path_mth, usecols=[3,4], index_col=[0], header=None, names=['t (s)', mth_name])
    # Correcting current for better scaling in image. Vscale/Iscale
    total = pd.concat([ch1, ch2*ch1_2_scale, mth], axis=1)
    # Plotting and saving the image
    plt.plot(total.index, total[ch1_name])
    plt.plot(total.index, total[ch2_name])
    plt.plot(total.index, total[mth_name])
    plt.xlabel('Tempo (s)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(total_path_name)
    plt.clf()
    return total
...
fft_iin_rc = plot_fourier_transform(
    2500,
    fwrrc_in.index.values,
    fwrrc_in['Iin (A) x 100'].values,
    "Corrente (A)",
    "img/lab3/iin_rc_fourier.png"
)

# Full Wave Rectfier Voltage Output Fourier Transform with RC Load
fft_vout_rc = plot_fourier_transform(
    2500,
    fwrrc_out.index.values,
    fwrrc_out['Vin (V)'].values,
    "Tensão (V)",
    "img/lab3/vout_rc_fourier.png"
)

# Full Wave Rectfier Current Output Fourier Transform with RC Load
fft_iout_rc = plot_fourier_transform(
    2500,
    fwrrc_out.index.values,
    fwrrc_out['Iin (A) x 100'].values,
    "Corrente (A)",
    "img/lab3/iout_rc_fourier.png"
)